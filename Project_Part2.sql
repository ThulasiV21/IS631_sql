...
END
GO
CREATE PROCEDURE tt347_Summer2019_Calc_GEO_Distance
@latitude VARCHAR(50),
...
Local_Site_Name =
CASE
WHEN Local_Site_Name IS NULL THEN (CAST(Site_Number AS VARCHAR(10)) + ' ' + City_Name)
WHEN Local_Site_Name = '' THEN (CAST(Site_Number AS VARCHAR(10)) + ' ' + City_Name)
WHEN Local_Site_Name = '????' THEN (CAST(Site_Number AS VARCHAR(10)) + ' ' + City_Name)
WHEN Local_Site_Name = '___________NO INFORMATION AT THIS TIME' THEN (CAST(Site_Number AS VARCHAR(10)) + ' ' + City_Name)
...
EXEC tt347_Summer2019_Calc_GEO_Distance 
@latitude = '31.968599',
@longitude = '-99.901813', 
@State = 'Texas', 
@rownum = 20
