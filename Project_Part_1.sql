...
SELECT MIN(Date_Local) AS [FISRT DATE]
    , MAX(Date_Local) AS [Last Date]
FROM Temperature
GO
...
WHERE t.State_Code = aqs.State_Code
    ...
    AND (
        ...
        OR (
            t.Average_Temp <= 105
            AND t.State_Code IN ('18', '26', '29', '30', '37', '38')
            )
        )
GO

-- Checking the row count of each state code in view
SELECT wv.State_Code
    , COUNT(*)
...
BEGIN
    ...
    SELECT wv.State_Name
        ...
        , RANK() OVER (
            ORDER BY Avg(average_temp) DESC
            ) Rank
    FROM temperature t
        , Weather_View wv
...
FROM (
    SELECT state_code
        ...
        , Row_number() OVER (
            ...
            , county_code
            , site_num
            , date_local ORDER BY state_code
                , county_code
                , site_num
                , date_local
            ) AS row_numb
...
FROM (
    SELECT state_code
        ...
        , Row_number() OVER (
            PARTITION BY state_code
            , county_code
            , Site_Number ORDER BY state_code
                , county_code
                , Site_Number
            ) AS row_numb
    FROM AQS_Sites
...
CREATE UNIQUE CLUSTERED INDEX IDX_Weather_View ON Weather_View (
    State_Code
    , County_Code
    , Site_Number
    , Date_Local
...
FROM (
    SELECT avg(t.Average_Temp) AS TEMP
        , A.State_Name
        , RANK() OVER (
            ORDER BY avg(t.Average_Temp) DESC
            ) AS State_Rank
    FROM Temperature t
        , (
    ...
    , (
        SELECT aqs.State_Name
            , aqs.City_Name
            , CAST(AVG(t.Average_Temp) AS DECIMAL(10, 6)) AS Average_Temperature
            , DENSE_RANK() OVER (
                PARTITION BY aqs.State_Name ORDER BY avg(t.Average_Temp) DESC
                ) AS City_Rank
        FROM Temperature t
            , AQS_Sites aqs
        WHERE t.State_Code = aqs.State_Code
            AND t.Site_Num = aqs.Site_Number
        GROUP BY aqs.City_Name
            , aqs.State_Name
        ) B
WHERE A.State_Name = B.State_Name
    AND A.State_Rank <= 15
ORDER BY A.State_Rank
    , B.City_Rank
GO
...
FROM (
    ...
    , (
        SELECT wv.State_Name
            ...
            , CAST(AVG(t.Average_Temp) AS DECIMAL(10, 6)) AS Average_Temperature
            , DENSE_RANK() OVER (
                PARTITION BY wv.State_Name ORDER BY avg(t.Average_Temp) DESC
                ) AS City_Rank
        FROM Temperature t
            , Weather_View wv
        WHERE t.State_Code = wv.State_Code
...
SELECT wv.City_Name
    , DATEPART(month, wv.Date_Local) [Month]
    , COUNT(t.Average_Temp) AS [# of Rows]
    , AVG(t.Average_Temp) AS [Average Temperature]
FROM Weather_View wv
    , Temperature t
WHERE wv.State_Code = t.State_Code
    AND wv.Site_Number = t.Site_Num
GROUP BY DATEPART(month, wv.Date_Local)
    , wv.City_Name
HAVING AVG(Average_Temp) >= 80
ORDER BY [Month]
GO
...
WITH Distribution_List
AS (
    SELECT wv.City_Name
        , CAST(t.Average_Temp AS DECIMAL(10, 6)) AS [Average Temp]
        , CAST(CUME_DIST() OVER (
                ORDER BY t.Average_Temp DESC
                ) AS DECIMAL(8, 6)) AS Temp_Cume_Dist
    FROM Weather_View wv
        , Temperature t
    WHERE wv.State_Code = t.State_Code
        AND wv.Site_Number = t.Site_Num
...
WHERE Temp_Cume_Dist >= 0.40
    AND Temp_Cume_Dist <= 0.60
...
-- focusing on.
SELECT B.City_Name
    , CAST(MIN(B.Avg_Temp) AS DECIMAL(10, 6)) AS [40 Percentile]
    , CAST(MAX(B.Avg_Temp) AS DECIMAL(10, 6)) AS [60 Percentile]
FROM (
    SELECT A.City_Name AS [City_Name]
        , A.Average_Temp AS [Avg_Temp]
        , A.CumeDist
    FROM (
        SELECT DISTINCT wv.City_Name
            , Average_Temp
            , CUME_DIST() OVER (
                PARTITION BY wv.City_Name ORDER BY Average_Temp
                ) AS CumeDist
        FROM Temperature AS t
        INNER JOIN Weather_View wv
            ON wv.State_Code = t.State_Code
                AND wv.County_Code = t.County_Code
                AND wv.Site_Number = t.Site_Num
        WHERE City_Name IN ('Boron', 'Springerville', 'Ludlow')
        ) AS A
    WHERE CumeDist >= 0.40
        AND CumeDist <= 0.60
    ) AS B
...
-- Question 13: You remember from your statistics classes that to get a smoother distribution of the temperatures and 
...
SELECT A.City_Name
    , A.[Day of the Year]
    , AVG(A.Avg_Temp) OVER (
        ORDER BY A.[Day of the Year] rows BETWEEN 3 preceding AND 1 following
        ) AS Rolling_Avg_Temp
FROM (
    SELECT wv.City_Name
        , DATEPART(DAY, wv.Date_Local) AS [Day of the Year]
        , AVG(t.Average_Temp) AS Avg_Temp
    FROM Weather_View wv
        , Temperature t
    WHERE wv.State_Code = t.State_Code
        AND wv.Site_Number = t.Site_Num
        AND wv.City_Name IN ('Boron', 'Springerville', 'Ludlow')
    GROUP BY wv.City_Name
        , DATEPART(DAY, wv.Date_Local)
    ) A
